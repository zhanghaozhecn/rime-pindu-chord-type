...
# 简介
拼读并击是一种简单高效的中文输入方案，主要编码成分为带声调的音节，难度低于五笔，词语重码数量接近高性能形码，具体指标见下表。并击是一种多指同时按键的输入方式，一击可以输入大量信息，拼读并击的一击可以打出一个完整的带声调音节。本文将介绍如何将音节拆成字音元素，从字中提取字形元素，元素和键位之间的映射，字和词的打法。
...
1. 将变形后的拼音从声母（如果有）和i/u/ü（如果有）后切开。前面的部分称为声介，后面的部分称为韵调。若声介为空（没有声母和i/u/ü），用#表示，如ai4→#+ai4。若韵调只有声调，用#1~#4表示，如du2→du+#2。
...
| sh/shi |   S  |    g   |  SF  |  mi  |  QF  |  cu  |  ZG  |
|   ji   |   A  |    t   |  AF  |  chu |  DV  |  lü  |  AS  |
| zh/zhi |   G  |   hu   |  ER  |  qü  |  SV  |  nu  |  SE  |
|   xi   |   R  |   zhu  |  WE  | s/si |  AV  |  nü  |  XD  |
|    u   |   E  |  r/ri  |  WR  |  ni  |  CF  |      |      |
|  b/bu  |   W  | ch/chi |  QR  | c/ci |  XF  |      |      |
|  z/zi  |   Q  |   du   |  CV  |  tu  |  ZF  |      |      |
|  f/fu  |   T  |   di   |  XC  |  xü  |  DR  |      |      |
|   gu   |   V  |    k   |  XV  |  jü  |  SR  |      |      |
|  m/mu  |   C  |    #   |  ZV  |  su  |  AR  |      |      |
