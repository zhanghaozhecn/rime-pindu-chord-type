...
#### 形旁是部首
取形旁作为部首。如，**势→力**（声旁“执”），**堂→土**（声旁“尚”）。

#### 形旁不是部首
取形旁中最明显的部首，占边角多的部首优先，同级按笔画顺序。如，**碧→石**（声旁“白”，取下边，不取左上角），**举→二**（声旁“兴”，誉→言，取下边，“十”不直观），**嚭→口**（声旁“否”，左上角“士”不足1/4，取左下角），**毙→歹**（声旁“比”，取左下角），**懿→心**（声旁“壹”，右上角不足1/4，取右下角），**雁→隹**（声旁“厂”，取右下角），**孵→卩**（声旁“孚”，无边角，取中间），**疍→⺊**（声旁“旦”，无边角，取中间）。

### 非形声字
特指除上文部首字和形声字之外的字。尽可能对非形声字进行**二分**，分割尽量直观，以二分后部件组字能力强为主，结构清晰为辅。例如，毂彀觳|嫠漦釐|嬴赢蠃|族旅旋|颍颖颎|糜磨靡|螣腾縢|倏修翛|辨辫辩|衰亵褒|斡翰乾|寝寤寐|爽夷奭|壹壶壸|庶度席，这几组字都是按照部件组字能力二分。根据二分的情况分类取部首。

#### 能二分且只有一个部件是部首
取这个部件。如，**贵→贝**，**辅→车**，**截→隹**。

#### 能二分且两个部件都是部首
一般取前部件。如，**顷→匕**。半包围结构的字取外面的部件。如，**凼→凵**，**连→辶**。

#### 不能二分、二分后部件都不是部首、或分成三部分以上更直观
取最明显的部首，占边角多的部首优先，同级按笔画顺序。如，**事→口**（无边角，取中间），**书→乛**（无多笔部首，取单笔部首，取右上角）；**蠲→皿**（左上角不足1/4，取左下角），**巢→木**（上边不是部首，取下边），**厾→厶**（外面是单笔画，取左下角），**临→囗**（取右下角）；**嬲→田**（取左上角），**掰→手**（取左边），**彝→廾**（取下边），**真→十**（取上边），**克→十**（取上边）。
